...
import { ArrowLeft, Clock, DollarSign, TrendingUp, TrendingDown, Building2, Zap } from 'lucide-react'
...
const PendingAuditsScreen = () => {
  ...
  const getTransactionColor = (type: string) => {
    switch (type) {
      ...
      case 'sell': return 'text-blue-700'
      case 'loan': return 'text-red-700'
      case 'market_event': return 'text-amber-700'
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900">Pending Audits</h1>
              <p className="text-sm text-gray-600">{pendingReviews.length} transaction{pendingReviews.length !== 1 ? 's' : ''} to review</p>
            </div>
          </div>
        </div>
      </div>
